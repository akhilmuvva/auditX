...
contract AuditBadgeNFT is ERC721, Ownable {
    ...
    function mintBadge(
        ...
        tokenAuditData[newTokenId] = AuditMetadata({
            ...
            zkChecksPassed: zkChecksPassed
        });

        emit BadgeMinted(recipient, newTokenId, contractName, severityScore, ipfsCid, zkChecksPassed);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // Update threshold logic: CVSS >= 7.0 stays AMBER, any ZK FAIL + CVSS >= 5.0 -> force AMBER
        if (audit.severityScore >= 70 || (!audit.zkChecksPassed && audit.severityScore >= 50)) {
            shieldColor = "#F59E0B"; // Amber warning
            badgeGrade = "AMBER GUARD";
        }
        
        string memory zkOverlay = "";
        if (audit.zkChecksPassed && keccak256(bytes(badgeGrade)) == keccak256(bytes("EMERALD GUARD"))) {
            zkOverlay = "<g transform='translate(0, 30)'><path d='M190 200l10 10 20-20' fill='none' stroke='#10B981' stroke-width='3' stroke-linecap='round' stroke-linejoin='round'/><text x='200' y='225' fill='#10B981' font-size='10' font-weight='bold' text-anchor='middle'>ZK SECURED</text></g>";
        } else if (!audit.zkChecksPassed && keccak256(bytes(badgeGrade)) == keccak256(bytes("AMBER GUARD"))) {
            zkOverlay = "<g transform='translate(0, 30)'><circle cx='200' cy='200' r='15' fill='#F59E0B' opacity='0.2'/><text x='200' y='204' fill='#F59E0B' font-size='12' font-weight='bold' text-anchor='middle'>!</text><text x='200' y='225' fill='#F59E0B' font-size='9' font-weight='bold' text-anchor='middle'>ZK FAIL</text></g>";
        }
        ...
        string memory json = string(abi.encodePacked(
            ...
            "{\"trait_type\": \"ZK Checks Passed\", \"value\": \"", audit.zkChecksPassed ? "True" : "False", "\"},",
